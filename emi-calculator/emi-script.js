...
const selectedHypotheticalScenario=()=>document.querySelector('input[name="hypothetical-scenario"]:checked')?.value || hypotheticalScenario;
...
function renderResults(result){
  ...
  displayInsights(insights,result.breakEvenMonth,modifiedLoan.totalMonths);
...
function resetHypotheticalComparison(){const activeScenario=selectedHypotheticalScenario(); hypotheticalScenario=activeScenario; const card=$('hypothetical-impact-card'); card.classList.add('hidden'); card.classList.remove('scenario-flash'); setText('hypothetical-highlight','Try a hypothetical prepayment to compare it against your current actual plan.'); setText('scenario-selection-label',activeScenario==='reduce-emi'?'Showing: Reduce EMI Scenario':'Showing: Reduce Tenure Scenario'); setText('scenario-applicability-note',''); setReduceEmiAvailability(false,''); setText('hypothetical-amount',formatCurrency(0)); setText('hypothetical-date',''); setText('scenario-current-emi',formatCurrency(0)); setText('scenario-current-interest',formatCurrency(0)); setText('scenario-current-tenure','0m'); setText('scenario-hypothetical-emi',formatCurrency(0)); setText('scenario-hypothetical-interest',formatCurrency(0)); setText('scenario-hypothetical-tenure','0m');}
function formatScenarioTenure(totalMonths,displayMonths){return fmtTenure(displayMonths??totalMonths);}
function setReduceEmiAvailability(disabled,message){const reduceEmi=$('scenario-reduce-emi'); if(!reduceEmi) return; reduceEmi.disabled=Boolean(disabled); if(disabled&&reduceEmi.checked){$('scenario-reduce-tenure').checked=true; hypotheticalScenario='reduce-tenure';} setText('scenario-applicability-note',disabled?message:'');}
function renderHypotheticalComparison(baseLoan,impact){const activeScenario=selectedHypotheticalScenario(); hypotheticalScenario=activeScenario; const card=$('hypothetical-impact-card'); if(!baseLoan||!impact){resetHypotheticalComparison(); return;} setReduceEmiAvailability(impact.reduceEmiApplicable===false,impact.reduceEmiDisabledMessage||''); const effectiveScenario=(impact.reduceEmiApplicable===false&&activeScenario==='reduce-emi')?'reduce-tenure':activeScenario; const selectedScenario=impact.comparisonScenarios?.[effectiveScenario] || impact.comparisonScenarios?.['reduce-tenure'] || impact.resultingLoan; if(!selectedScenario){resetHypotheticalComparison(); return;} const currentDisplayMonths=impact.displayCurrentMonths??baseLoan.totalMonths; const selectedDisplayMonths=selectedScenario.displayMonths??selectedScenario.totalMonths; const tenureSaved=Math.max(0,currentDisplayMonths-selectedDisplayMonths); const interestSaved=Math.max(0,baseLoan.totalInterest-selectedScenario.totalInterest); const remainingPrincipal=Number(impact.remainingPrincipalAfterPrepayment)||0; const finalPayment=Number(selectedScenario.finalPayment)||0; const showFinalPayment=impact.status==='near-closure'&&finalPayment>0; const statusMessage=impact.message||`Remaining principal after prepayment: ${formatCurrency(remainingPrincipal)}.`; const finalPaymentMessage=showFinalPayment?` Final payment: ${formatCurrency(finalPayment)} instead of the regular EMI.`:''; card.classList.remove('hidden'); card.classList.remove('scenario-flash'); void card.offsetWidth; card.classList.add('scenario-flash'); setText('scenario-selection-label',effectiveScenario==='reduce-emi'?'Showing: Reduce EMI Scenario':'Showing: Reduce Tenure Scenario'); setText('hypothetical-highlight',`${statusMessage}${finalPaymentMessage} You save ${formatCurrency(interestSaved)} in interest over the remaining loan period and ${fmtTenure(tenureSaved)}.`); setText('hypothetical-amount',formatCurrency(impact.amount)); setText('hypothetical-date',formatDate(impact.date)); setText('scenario-current-emi',formatCurrency(baseLoan.emi)); setText('scenario-current-interest',formatCurrency(baseLoan.totalInterest)); setText('scenario-current-tenure',formatScenarioTenure(baseLoan.totalMonths,currentDisplayMonths)); setText('scenario-hypothetical-emi',formatCurrency(selectedScenario.emi)); setText('scenario-hypothetical-interest',formatCurrency(selectedScenario.totalInterest)); setText('scenario-hypothetical-tenure',formatScenarioTenure(selectedScenario.totalMonths,selectedDisplayMonths));}
...
function renderActualAdvisor(result,monthlyIncome){const {originalLoan,modifiedLoan,prepaymentImpact,reduceEmiLoan,loanProgress}=result; const ratio=monthlyIncome&&monthlyIncome>0?originalLoan.emi/monthlyIncome:null; const recommendation=ratio!==null&&ratio>0.35?'Recommendation: Reduce EMI to ease monthly cash flow.':'Recommendation: Reduce tenure to maximize savings.'; const timing=loanProgress.paidEmis<originalLoan.totalMonths*0.3?'Best time to prepay - maximum savings.':loanProgress.paidEmis<originalLoan.totalMonths*0.7?'Moderate benefit.':'Lower impact - most interest is already paid.'; renderAdvisorCard({title:`Your actual prepayment plan is already saving ${formatCurrency(prepaymentImpact.interestSaved)}.`,interestSaved:prepaymentImpact.interestSaved,tenureSaved:prepaymentImpact.monthsReduced,emiReduction:Math.max(0,originalLoan.emi-reduceEmiLoan.emi),recommendation,timing,helperText:'Earlier prepayments save more interest.',rows:[{label:'Current',emi:originalLoan.emi,interest:originalLoan.totalInterest,tenure:originalLoan.totalMonths},{label:'Reduce Tenure',emi:modifiedLoan.emi,interest:modifiedLoan.totalInterest,tenure:modifiedLoan.totalMonths},{label:'Reduce EMI',emi:reduceEmiLoan.emi,interest:reduceEmiLoan.totalInterest,tenure:reduceEmiLoan.totalMonths}],emiReductionFrom:originalLoan.emi,emiReductionTo:reduceEmiLoan.emi});}
function renderSmartAdvisor(advisor,sourceLabel){const recommendation=advisor.recommendation==='Reduce EMI'?'Recommendation: Reduce EMI to ease monthly cash flow.':advisor.recommendation==='Loan Closed'?'Recommendation: This prepayment pays off the loan in full.':'Recommendation: Reduce tenure to maximize savings.'; const summaryMessage=advisor.message?`${advisor.message} `:''; const remainingPrincipal=advisor.currentState?.remainingPrincipalAfterPrepayment; const helperText=advisor.helperText; const titleSuffix=remainingPrincipal!==undefined?` Remaining principal after prepayment: ${formatCurrency(remainingPrincipal)}.`:''; const finalPayment=Number(advisor.scenarios.reduceTenure?.finalPayment)||0; const showFinalPayment=advisor.reduceEmiApplicable===false&&advisor.recommendation==='Reduce Tenure'&&finalPayment>0; const finalPaymentMessage=showFinalPayment?` Final payment: ${formatCurrency(finalPayment)} instead of the regular EMI.`:''; const rows=[{label:'Current',emi:advisor.scenarios.current.emi,interest:advisor.scenarios.current.totalInterest,tenure:advisor.scenarios.current.totalMonths,displayTenure:advisor.scenarios.current.displayMonths},{label:'Reduce Tenure',emi:advisor.scenarios.reduceTenure.emi,interest:advisor.scenarios.reduceTenure.totalInterest,tenure:advisor.scenarios.reduceTenure.totalMonths,displayTenure:advisor.scenarios.reduceTenure.displayMonths}]; if(advisor.reduceEmiApplicable!==false){rows.push({label:'Reduce EMI',emi:advisor.scenarios.reduceEmi.emi,interest:advisor.scenarios.reduceEmi.totalInterest,tenure:advisor.scenarios.reduceEmi.totalMonths,displayTenure:advisor.scenarios.reduceEmi.displayMonths});} renderAdvisorCard({title:`${sourceLabel}: ${summaryMessage}You save up to ${formatCurrency(advisor.scenarios.reduceTenure.interestSaved)} in interest over the remaining loan period.${titleSuffix}${finalPaymentMessage}`,interestSaved:advisor.scenarios.reduceTenure.interestSaved,tenureSaved:advisor.scenarios.reduceTenure.monthsSaved,recommendation,timing:advisor.reduceEmiApplicable===false?(advisor.reduceEmiDisabledMessage||advisor.timingInsight):advisor.timingInsight,helperText,rows,emiReductionFrom:advisor.scenarios.current.emi,emiReductionTo:advisor.reduceEmiApplicable===false?advisor.scenarios.current.emi:advisor.scenarios.reduceEmi.emi});}
function formatEmiReduction(fromEmi,toEmi){const from=Math.max(0,Number(fromEmi)||0); const to=Math.max(0,Number(toEmi)||0); const reduction=Math.max(0,from-to); return reduction>0?`EMI reduces from ${formatCurrency(from)} ${String.fromCharCode(8594)} ${formatCurrency(to)} (${String.fromCharCode(8595)} ${formatCurrency(reduction)})`:'No EMI reduction in this scenario.';}
function renderAdvisorCard({title,interestSaved,tenureSaved,recommendation,timing,helperText,rows,emiReductionFrom=0,emiReductionTo=0}){$('smart-prepayment-advisor-card').classList.remove('hidden'); setText('advisor-highlight',title); animateValue($('advisor-interest-saved'),interestSaved,v=>formatCurrency(v)); animateValue($('advisor-tenure-saved'),tenureSaved,v=>fmtTenure(Math.round(v))); setText('advisor-emi-reduction',formatEmiReduction(emiReductionFrom,emiReductionTo)); setText('advisor-recommendation',recommendation); setText('advisor-timing',timing); setText('advisor-helper-text',helperText); $('advisor-comparison-body').innerHTML=rows.map(r=>`<tr><td>${r.label}</td><td>${formatCurrency(r.emi)}</td><td>${formatCurrency(r.interest)}</td><td>${formatScenarioTenure(r.tenure,r.displayTenure)}</td></tr>`).join('');}
...
function displayInsights(insights,breakEvenMonth,totalMonths){const list=$('emi-insight-list'); list.innerHTML=''; if(!insights||insights.length===0){displayMessage('info','No specific insights generated for this scenario. Your loan appears to be well-managed.','no-insights-tip'); return;} insights.forEach(insight=>{const li=document.createElement('li'); li.className=`insight-item insight-item-${insight.type}`; li.innerHTML=`<span class="insight-icon">${insight.icon}</span><div class="insight-content"><h4 class="insight-title">${insight.title}</h4><p class="insight-message">${insight.message}</p>${insight.action?`<p class="insight-action"><strong>Action:</strong> ${insight.action}</p>`:''}</div>`; list.appendChild(li);}); $('break-even-point-display').textContent=breakEvenMonth!==undefined&&breakEvenMonth!==null?(totalMonths!==undefined&&totalMonths!==null&&totalMonths<=12?'Short-term loan insight: A larger share of each EMI goes toward principal early on.':`Break-even Point: Principal paid exceeds interest paid at month ${breakEvenMonth}.`):'Break-even Point: Not reached within the loan tenure, or principal always exceeded interest.';}
