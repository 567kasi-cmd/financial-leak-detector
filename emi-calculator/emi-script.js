...
function renderHypotheticalComparison(baseLoan,impact){const activeScenario=selectedHypotheticalScenario(); hypotheticalScenario=activeScenario; const card=$('hypothetical-impact-card'); if(!baseLoan||!impact){resetHypotheticalComparison(); return;} const selectedScenario=impact.comparisonScenarios?.[activeScenario] || impact.comparisonScenarios?.['reduce-tenure'] || impact.resultingLoan; if(!selectedScenario){resetHypotheticalComparison(); return;} const tenureSaved=Math.max(0,baseLoan.totalMonths-selectedScenario.totalMonths); const interestSaved=Math.max(0,baseLoan.totalInterest-selectedScenario.totalInterest); const remainingPrincipal=Number(impact.remainingPrincipalAfterPrepayment)||0; const statusMessage=impact.message||`Remaining principal after prepayment: ${formatCurrency(remainingPrincipal)}.`; card.classList.remove('hidden'); card.classList.remove('scenario-flash'); void card.offsetWidth; card.classList.add('scenario-flash'); setText('scenario-selection-label',activeScenario==='reduce-emi'?'Showing: Reduce EMI Scenario':'Showing: Reduce Tenure Scenario'); setText('hypothetical-highlight',`${statusMessage} You save ${formatCurrency(interestSaved)} and ${fmtTenure(tenureSaved)}.`); setText('hypothetical-amount',formatCurrency(impact.amount)); setText('hypothetical-date',formatDate(impact.date)); setText('scenario-current-emi',formatCurrency(baseLoan.emi)); setText('scenario-current-interest',formatCurrency(baseLoan.totalInterest)); setText('scenario-current-tenure',fmtTenure(baseLoan.totalMonths)); setText('scenario-hypothetical-emi',formatCurrency(selectedScenario.emi)); setText('scenario-hypothetical-interest',formatCurrency(selectedScenario.totalInterest)); setText('scenario-hypothetical-tenure',fmtTenure(selectedScenario.totalMonths));}
